...
Girilen bilye ağırlıklarının birbirine göre karşılaştırılması ve farklı olan bilyenin bulunmasını sağlayan algoritma şu şekilde kurulmuştur:
1. İlk olarak alınan ilk üç değer c,k ve z adlı üç değişkene atanmıştır. 
2. Daha sonra bu değişkenler kendi aralarında karşılaştırılarak farklı olan değerler(dif_value1, dif_value2 vb.) ve olmayan değerler (base_value4, base_value1 vb.) bulunmuştur.
3. Daha sonra bu değerlere göre hafiflik, ağırlık, ağırlık farkı ve yüzdesi gibi diğer değerler bulunmuştur.

* "#print('......')" şeklindeki commentler olası bir hata durumunda hata tespitini kolaylaştırmak için konmuştur. (yazar burada kodu okuyanın kendisi gibi debugging bilmediğini varsaymaktadır :)
...
# loop ve if/elif/else statement içeren algoritma kısmı

while cevap_v1 == 'e' or cevap_v1 == 'E':
    kutu_bilye_sayi = int(
        input('Lütfen kutuda bulunan bilye sayısını giriniz (gireceğiniz değer ondan küçük olmamalıdır): '))
    ...
    for i in range(1, kutu_bilye_sayi + 1):
        bilye_agirlik = int(
            input('Lütfen bilyenin ağırlığını giriniz (gireceğiniz değer sıfırdan büyük ve mg biriminde olmalıdır):  '))
        while bilye_agirlik <= 0:
            bilye_agirlik = int(input('Lütfen bilyenin ağırlığını sıfırdan büyük olacak şekilde giriniz:  '))
        if i == 1:
            c = bilye_agirlik  # ilk girdiye atanan sabit
        if i == 2:
            k = bilye_agirlik  # ikinci girdiye atanan sabit
        if i == 3:
            z = bilye_agirlik  # üçüncü girdiye atanan sabit, ve ilk üç girdiyi aldıktan sonra ilk üç girdiyi kendi arasında karşılaştıran ve dif_value ve base_value belirleyen birinci kısım algoritma
            if c == k and k == z:
                base_value1 = c
            elif c == k and c != z:
                base_value2 = c
        ...
        if i > 3:  # ilk üç girdiden sonra girilen diğer değerleri ilk üç girdide belirlenen dif_value ve base_value değerlerine göre karşılaştıran ve yanlış sayısına göre for döngüsünden çıkan veya çıkmayan ikinci kısım kod
        ...
        if cevap_v3 == 1:
            break
        if i == kutu_bilye_sayi:  # for döngüsü bitiminde istenen ilk birkaç çıktı için boolean tipi değişkenlerle kontrol edilerek yazdırılacak olan ilk kısım print
            if value != 0:
                print_onay = False
            ...
            if print_onay == True:
                ...
                # en çok bilye olan kutudaki bilye sayısı ve ağırlığı ile en ağır bilye olan kutudaki bilye sayısı ve ağırlığını hesaplayan üçüncü kısım kod
...
print('\n')
print(
    f"Farklı bilyenin diğerlerinden ağır olduğu kutulardaki ağırlık farkı ortalaması: {agirlik_farki_ortalama_agir:,.2f} ve ağırlık farkı yüzdesini ortalaması: %{agirlik_yuzde_ortalama_agir:,.2f}")
print('\n')
print(
    f"Farklı bilyenin diğerlerinden hafif olduğu kutulardaki ağırlık farkı ortalaması: {agirlik_farki_ortalama_hafif:,.2f} ve ağırlık farkı yüzdesini ortalaması: %{agirlik_yuzde_ortalama_hafif:,.2f}")
print('\n')
